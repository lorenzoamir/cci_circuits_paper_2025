...
motifs = motifs.query('Type in ["3_clique", "4_clique"]').index
...
def survival(interactions, df):  
    ...
    try:
        ...
        logrank_pval = logrank_test(low_expression_group['OS.time'], high_expression_group['OS.time'],
                               event_observed_A=low_expression_group['OS'],
                               event_observed_B=high_expression_group['OS']).p_value
...
ccc_results = pd.DataFrame(index=ccc, columns=['interaction', 'logrank_pval', 'hazard_ratio', 'n_patients_low', 'n_patients_high'])
for interaction in ccc:
    result = survival(interaction, df)
    ccc_results.loc[interaction] = { 
        'interaction': interaction,
        'logrank_pval': result['pval'],
...
ccc_results = ccc_results.sort_values('hazard_ratio', ascending=False)

# Also add crosstalk interactions from the motifs file
crosstalk_results = pd.DataFrame(index=motifs, columns=['interaction', 'logrank_pval', 'hazard_ratio', 'n_patients_low', 'n_patients_high'])
...
all_results = all_results.sort_values('logrank_pval')
