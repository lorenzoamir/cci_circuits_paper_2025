#!/bin/bash
#PBS -N aggr_lymphatic_tissue
#PBS -l select=1:ncpus=2:ngpus=0:mem=20gb
#PBS -q q02anacreon
mkdir -p /home/lnemati/pathway_crosstalk/code/8.5_immunotherapy_non_cci_cliques
cd /home/lnemati/pathway_crosstalk/code/8.5_immunotherapy_non_cci_cliques
eval "$(/cluster/shared/software/miniconda3/bin/conda shell.bash hook)"
conda activate WGCNA
python aggregate.py --inputdir /home/lnemati/pathway_crosstalk/results/immunotherapy/non_cci_cliques/individual_interactions_and_motifs/lymphatic_tissue
exit 0
